...
function MainChat({ apiConfig, serverConfig }) {
  ...
  const sendMessage = async (retryCount = 0) => {
    ...
    if (!apiConfig?.apiKey || !CryptoUtils.validateKey(apiConfig.apiKey)) {
      setError('Ошибка: Неверная конфигурация API');
      return;
    }
    ...
    try {
      ...
      // ИСПОЛЬЗУЕМ ПРОКСИ ВМЕСТО ПРЯМОГО ВЫЗОВА GROQ API
      const response = await fetch(`${apiConfig.baseURL}${apiConfig.endpoint}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          // Authorization заголовок больше не нужен
        },
      ...
      if (!response.ok) {
        ...
        throw new Error(`API ошибка ${response.status}: ${errorData.error || 'Unknown error'}`);
      }
      ...
      if (error.name === 'AbortError') {
        ...
        errorMessage = 'Неверный API-ключ. Проверьте настройки API на сервере.';
